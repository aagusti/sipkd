...
from ...models.ar import ARInvoiceBphtb
...
class view_invoice(base_view):
    @view_config(route_name="ar-invoice-bphtb", renderer="templates/ar-invoice-bphtb/list.pt",
                 permission="ar-invoice-bphtb")
    def view_list(self):
...
# Action #
##########
@view_config(route_name='ar-invoice-bphtb-act', renderer='json',
             permission='read')
def view_act(request):
    ...
    if url_dict['act']=='grid':
        ...
        if url_dict['act']=='grid':
            ...
            query = DBSession.query(ARInvoiceBphtb.id,
                      ...
                      ).filter(ARInvoiceBphtb.tahun==ses['tahun'],
                               between(ARInvoiceBphtb.tgl_tetap, ses['tanggal'], ses['tanggal_to']),
...
def route_list(request):
    return HTTPFound(location=request.route_url('ar-invoice-bphtb'))
...
@view_config(route_name='ar-invoice-bphtb-add', renderer='templates/ar-invoice-bphtb/add.pt',
             permission='ar-invoice-bphtb-add')
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            if not controls_dicted['kode']=='':
                ...
                b = a['kode']
                c = "%s" % b
                cek  = DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.kode==c).first()
                if cek :
                    request.session.flash('Kode ARInvoice sudah ada.', 'error')
                    return HTTPFound(location=request.route_url('ar-invoice-bphtb-add'))
            ...
            return HTTPFound(location=request.route_url('ar-invoice-bphtb-edit',id=row.id))
...
def query_id(request):
    return DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.id==request.matchdict['id'])
...
@view_config(route_name='ar-invoice-bphtb-edit', renderer='templates/ar-invoice-bphtb/add.pt',
             permission='ar-invoice-bphtb-edit')
def view_edit(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            cek = DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.kode==c).first()
            if cek:
                kode1 = DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.id==uid).first()
                d     = kode1.kode
                if d!=c:
                    request.session.flash('Kode ARInvoiceBphtb sudah ada', 'error')
                    return HTTPFound(location=request.route_url('ar-invoice-bphtb-edit',id=row.id))
...
# Delete #
##########
@view_config(route_name='ar-invoice-bphtb-delete', renderer='templates/ar-invoice-bphtb/delete.pt',
             permission='ar-invoice-bphtb-delete')
def view_delete(request):
    ...
    if request.POST:
        if 'hapus' in request.POST:
            ...
            DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.id==request.matchdict['id']).delete()
            DBSession.flush()
...
def query_post_id(id):
    return DBSession.query(ARInvoiceBphtb).filter(ARInvoiceBphtb.id==id)

@view_config(route_name='ar-invoice-bphtb-post', renderer='json',
             permission='ar-invoice-bphtb-post')
...
# CSV #
##########
@view_config(route_name='ar-invoice-bphtb-csv', renderer='csv',
             permission='ar-invoice-bphtb-csv')
def view_csv(request):
    ses = request.session
    req = request
    params   = req.params
    url_dict = req.matchdict
    q = DBSession.query(ARInvoiceBphtb.id,
              ARInvoiceBphtb.unit_kd,
              ARInvoiceBphtb.kode,
              ARInvoiceBphtb.nama,
              ARInvoiceBphtb.rekening_kd,
