...
from ...models.ar import ARPaymentBphtb
...
@view_config(route_name="ar-payment-bphtb", renderer="templates/ar-payment-bphtb/list.pt",
             permission="ar-payment-bphtb")
...
# Action #
##########    
@view_config(route_name='ar-payment-bphtb-act', renderer='json',
             permission='read')
def view_act(request):
    ...
    if url_dict['act']=='grid':
        ...
        if url_dict['act']=='grid':
            ...
            query = DBSession.query(ARPaymentBphtb.id,
                    ...
                    #).order_by(ARPaymentBphtb.kode.asc()
                    #)
            rowTable = DataTables(req, ARPaymentBphtb, query, columns)
...
def route_list(request):
    return HTTPFound(location=request.route_url('ar-payment-bphtb'))
...
@view_config(route_name='ar-payment-bphtb-add', renderer='templates/ar-payment-bphtb/add.pt',
             permission='ar-payment-bphtb-add')
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            if not controls_dicted['kode']=='':
                ...
                b = a['kode']
                c = "%s" % b
                cek  = DBSession.query(ARPaymentBphtb).filter(ARPaymentBphtb.kode==c).first()
                if cek :
...
def query_id(request):
    return DBSession.query(ARPaymentBphtb).filter(ARPaymentBphtb.id==request.matchdict['id'])
...
@view_config(route_name='ar-payment-bphtb-edit', renderer='templates/ar-payment-bphtb/add.pt',
             permission='ar-payment-bphtb-edit')
def view_edit(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            cek = DBSession.query(ARPaymentBphtb).filter(ARPaymentBphtb.kode==c).first()
            if cek:
                kode1 = DBSession.query(ARPaymentBphtb).filter(ARPaymentBphtb.id==uid).first()
                d     = kode1.kode
                if d!=c:
                    request.session.flash('Kode ARPaymentBphtb sudah ada', 'error')
                    return HTTPFound(location=request.route_url('ar-payment-edit',id=row.id))
...
# Delete #
##########    
@view_config(route_name='ar-payment-bphtb-delete', renderer='templates/ar-payment-bphtb/delete.pt',
             permission='ar-payment-bphtb-delete')
