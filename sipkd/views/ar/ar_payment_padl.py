...
from ...models.ar import ARPaymentTrans
...
@view_config(route_name="ar-payment-padl", renderer="templates/ar-payment-padl/list.pt",
             permission="ar-payment-padl")
...
# Action #
##########    
@view_config(route_name='ar-payment-padl-act', renderer='json',
             permission='read')
def view_act(request):
    ...
    if url_dict['act']=='grid':
        ...
        if url_dict['act']=='grid':
            ...
            query = DBSession.query(ARPaymentTrans.id,
                      ARPaymentTrans.unit_kd,
                      ARPaymentTrans.kode,
                      ARPaymentTrans.nama,
                      ARPaymentTrans.rekening_kd,
                      ARPaymentTrans.rekening_nm,
                      ARPaymentTrans.pokok,
                      ARPaymentTrans.denda,
                      ARPaymentTrans.bunga,
                      ARPaymentTrans.posted,
                      ARPaymentTrans.tgl_trans,
                      ARPaymentTrans.tgl_validasi,
                      ).filter(ARPaymentTrans.tahun==ses['tahun'],
                               between(ARPaymentTrans.tgl_trans, ses['tanggal'], ses['tanggal_to']),
                               ARPaymentTrans.posted==ses['posted']
                      ) 
                    #).order_by(ARPaymentTrans.kode.asc()
                    #)
            rowTable = DataTables(req, ARPaymentTrans, query, columns)
...
def route_list(request):
    return HTTPFound(location=request.route_url('ar-payment-padl'))
...
@view_config(route_name='ar-payment-padl-add', renderer='templates/ar-payment-padl/add.pt',
             permission='ar-payment-padl-add')
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            if not controls_dicted['kode']=='':
                ...
                b = a['kode']
                c = "%s" % b
                cek  = DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.kode==c).first()
                if cek :
...
def query_id(request):
    return DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.id==request.matchdict['id'])
...
@view_config(route_name='ar-payment-padl-edit', renderer='templates/ar-payment-padl/add.pt',
             permission='ar-payment-padl-edit')
def view_edit(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            cek = DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.kode==c).first()
            if cek:
                kode1 = DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.id==uid).first()
                d     = kode1.kode
                if d!=c:
                    request.session.flash('Kode ARPaymentTrans sudah ada', 'error')
                    return HTTPFound(location=request.route_url('ar-payment-edit',id=row.id))
...
# Delete #
##########    
@view_config(route_name='ar-payment-padl-delete', renderer='templates/ar-payment-padl/delete.pt',
             permission='ar-payment-padl-delete')
def view_delete(request):
    ...
    if request.POST:
        if 'hapus' in request.POST:
            ...
            DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.id==request.matchdict['id']).delete()
            DBSession.flush()
...
def query_post_id(id):
    return DBSession.query(ARPaymentTrans).filter(ARPaymentTrans.id==id)

@view_config(route_name='ar-payment-padl-post', renderer='json',
             permission='ar-payment-padl-post')
...
# UnPosting #
#############   
@view_config(route_name='ar-payment-padl-unpost', renderer='json',
             permission='ar-payment-padl-unpost')
...
def view_csv(request):
    ...
    params   = req.params
    url_dict = req.matchdict
    q = DBSession.query(ARPaymentTrans.id,
        ARPaymentTrans.unit_kd,
        ARPaymentTrans.kode,
        ARPaymentTrans.nama,
        ARPaymentTrans.rekening_kd,
